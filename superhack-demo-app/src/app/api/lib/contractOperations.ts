...
export async function checkVerification(pshandle: string | undefined) {
    try {
        ...
        const profile = await contract.getProfileByPshandle(pshandle);
        // console.log("profile checkver", profile);

        const profileExists = isProfilePopulated(profile);
        console.log( "sup", profileExists);

        return { profile, isProfile: profileExists };
    } catch (error) {
...
// Utility function to check if the profile is populated
function isProfilePopulated(profile: any) {
    const [
        pshandle,
        twitter,
        instagram,
    ...
    // Check if any of the handles is non-empty
    return (
        twitter[0] !== '' ||
        instagram[0] !== '' ||
        facebook[0] !== '' ||
